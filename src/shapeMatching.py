...
class ShapeMatching:
    ...
    @staticmethod
    def ShapeMatchKMP(img_path: str, side: str, dataset_pattern: dict[str, dict[str, str]]) -> list[str]:
        """
        ...
        for image_name, pattern_data in dataset_pattern.items():
            ...
            for pattern_side, pattern in pattern_data.items():
                ...
                for sub_pattern in sub_patterns:
                    if KMP.KMPSearch(pattern, sub_pattern):
                        total_count += 1
                ...
                print(f"{image_name} {pattern_side} : {total_count}")

        if max_count > 6:
            if len(best_matches) == 1:
                return best_matches[0]

        return ShapeMatching.ShapeMatchLD(img_path, side, dataset_pattern)
    
    @staticmethod
    def ShapeMatchBM(img_path: str, side: str, dataset_pattern: dict[str, dict[str, str]]) -> list[str]:
        """
    ...
    @staticmethod
    def ShapeMatchLD(img_path: str, side: str, dataset_pattern: dict[str, dict[str, str]]) -> str:
        """
